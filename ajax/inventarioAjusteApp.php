...
require_once "../modelos/inventarioAjusteApp.php";
		...
		$rspta=$inventarioAjuste->mostrarDetalle($idAjusteStock);
 		...
 		while ($reg=$rspta->fetchObject()){
 			...
 			$data[]=array(
 				...
 				"1"=>utf8_encode($reg->ns),
				"2"=>utf8_encode($reg->productos),
				"3"=>'<input class="text-right" type="text" name="StockPromedio[]"  style="width:100%;" onblur="actualizarCantidadCajaAjuste(this,\''.$reg->idArticulo.'\',\''.$reg->Deposito_idDeposito.'\',\''.$reg->stockInicial.'\',\''.$contPrecio.'\')"  id="StockPromedio[]" style="width:100%;" value="'.$reg->stockInicial.'" />',
				"4"=>$reg->totalVendido,
				"5"=>$reg->stockInicial - $reg->totalVendido,		
				"6"=>number_format($reg->montoVendido). ' Gs.',		
 				);
 			$contPrecio++;
 		}
 		$results = array(
...
"8"=>'<input type="text" name="comentario[]"  style="width:100%;" onblur="actualizarComentario(this,\''.$reg->idcierrecostodetalle.'\')"  id="comentario[]" style="width:100%;" value="'.$reg->comentario.'" />',
		...
		$rspta=$inventarioAjuste->listar($Deposito_idDeposito, $fi, $ff, $estado, $Sucursal_idSucursal, $Categoria_idCategoria);
 		...
 		$url='../reportes/exTicket.php?id=';
 			$data[]=array(
 				...
 				"1"=>$reg->deposito, 
 				"2"=>$reg->sucursal,
 				"3"=>$reg->Habilitacion_idHabilitacion,
	...
	case 'actualizarCantidadCajaAjuste':
		...
		$rspta=$inventarioAjuste->actualizarCantidadCajaAjuste($cantidad, $idArticulo,$Deposito_idDeposito);
 		//$data= 1;
 		echo json_encode($rspta);
