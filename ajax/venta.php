...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($idVenta)){
			$rspta=$venta->insertar($Direccion_idDireccion, $entrega, $Moneda_idMoneda, $tasaCambio, $tasaCambioBases,$Cliente_idCliente,$Habilitacion_idHabilitacion,$Deposito_idDeposito,$TerminoPago_idTerminoPago,$tipo_comprobante,$nroFactura,$fechaFactura,$fechaVencimiento,$timbrado,$vtoTimbrado,$totalImpuesto,$total,$totalNeto,$usuarioInsercion,$serie,$cuotas,$giftCard,$nroGiftCard,$clienteGiftCard,$Empleado_idEmpleado,$OrdenVenta_idOrdenVenta,$_POST["idarticulo"],$_POST["descripcion"],$_POST["cantidad"],$_POST["cantidadStock"],$_POST["precioVenta"],$_POST["impuesto"],$_POST["TipoImpuesto_idTipoImpuesto"],$_POST["importe_detalle"],$_POST["tipopago"], $_POST["nroReferencia"], $_POST["capital"], $_POST["interes"], $_POST["descuento"], $_POST["Banco_idBanco"], $_POST["importe_detalle_pagado"],$_POST['moneda'],$_POST['tasa'],$remision);
						...
						while ($reg = $rspta->fetchObject())
								{
									...
									<td>
										...
										<input type="hidden" name="cantidadStock[]" id="cantidadStock[]" value="'.$reg->cantidadStock.'">
...
"0"=>'<button class="btn btn-warning" onclick="agregarDetalle('.$reg->idArticulo.',\''.($reg->na).'\',\''.$reg->precioTL.'\',\''.$reg->pi.'\',\''.$reg->capital.'\',\''.$interes.'\',\''.$reg->stock.'\',\''.$reg->descant.'\')"><span class="fa fa-plus"></span></button>', 
 "1"=>($reg->na),
 				"2"=>($reg->cn),
 				"3"=>$reg->codigoBarra,
 				"4"=>$reg->precioTL,
	...
	case 'selectArticulosVenta':
		...
		$rspta=$articulo->listarActivosVenta('0',null,'1',0,$buscar_art);
 		//Vamos a declarar un array 
 		$data= Array();
