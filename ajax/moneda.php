...
switch ($_GET["op"]){
	...
	case 'mostrar':
		...
		if ($rspta === false) {
			echo json_encode(array('error' => 1, 'mensaje' => 'Registro no encontrado'));
			break;
		}
		// Asegurar objeto para JSON (fetch puede devolver array asociativo)
		echo json_encode(is_array($rspta) ? (object)$rspta : $rspta);
	break;
	...
	case 'selectMonedaTodos':

		$rspta = $moneda->listar();

		echo '<option value="%%">Todas las monedas</option>';	
	...
	case 'selectMonedaDenominacion':

		$rspta = $moneda->listarDenominacion($_POST['id']);
 
		while ($reg = $rspta->fetchObject())
				{
				echo '<option value=' . $reg->idDetalle . '>' . $reg->denominacion . '</option>';
				}
	break;		
