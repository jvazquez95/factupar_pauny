...
require_once "../modelos/NotaCreditoVenta.php";

$compra=new NotaCreditoVenta();
...
$Venta_idVenta=isset($_POST["Venta_idVenta"])? limpiarCadena($_POST["Venta_idVenta"]):"";
...
switch ($_GET["op"]){
	case 'guardaryeditar':
		...
		if (empty($idVenta)){
			$rspta=$compra->insertar($serie,$Persona_idPersona,$Habilitacion_idHabilitacion,$Deposito_idDeposito,$TerminoPago_idTerminoPago,$tipo_comprobante,$nroFactura,$fechaFactura,$timbrado,$vtoTimbrado,$totalImpuesto,$total,$totalNeto,$usuarioInsercion,$imagen,$Venta_idVenta,$_POST["idarticulo"],$_POST["cantidad"],$_POST["precio"],
				$_POST["impuesto"],$_POST["importe_detalle"],$_POST["tipopago"], $_POST["nroReferencia"], $tasaCambio, $tasaCambioBases, $Moneda_idMoneda);
			//$rspta=$compra->insertar($Persona_idPersona,$Habilitacion_idHabilitacion,$Deposito_idDeposito,$TerminoPago_idTerminoPago,$tipo_comprobante,$nroFactura,$fechaFactura,$fechaVencimiento,$timbrado,$vtoTimbrado,$totalImpuesto,$total,$totalNeto,$usuarioInsercion,$_POST["idarticulo"],$_POST["cantidad"],$_POST["preciocompra"],$_POST["impuesto"],$_POST["importe_detalle"],$_POST["tipopago"], $_POST["nroReferencia"]);
	...
	case 'anular':
		$rspta=$compra->anular($idVenta);
 		echo $rspta ? "Nota Credito anulada" : "Nota de Credito no se puede anular";
	break;

	case 'mostrar':
		$rspta=$compra->mostrar($idVenta);
...
<td><span name="subtotal" id="subtotal'.$contPrecio.'">'.$reg->total.'</span></td></tr>';
		...
		$rspta=$compra->listar();
 		...
 		while ($reg=$rspta->fetchObject()){
 			...
 			$data[]=array(
 				"0"=>
				''.
				(((!$reg->inactivo)?'<a target="_blank" href="'.$url.$reg->idNotaCreditoVenta.'"> <button class="btn btn-info"><i class="fa fa-print">Imprimir Nota de Credito</i></button></a>':
 					'')
					)
					,
 				"1"=>$reg->idNotaCreditoVenta.' - Nro Venta: '.$reg->Venta_idVenta,
 				"2"=>$reg->nroFactura,
 				"3"=>$reg->razonSocial,
 				"4"=>$reg->tipoComprobante,
 				"5"=>$reg->fechaTransaccion,
 				"6"=>number_format($reg->totalC,0,',','.'),
 				"7"=>'<button class="btn btn-primary" onclick="mostrarDetalleNC('.$reg->idNotaCreditoVenta.')">Ver detalle NC <i class="fa fa-eye"></i></button>',
	...
	case 'autorizarCompra':
		...
		$rspta=$compra->autorizarCompra($idVenta);
 		$data= 1;
