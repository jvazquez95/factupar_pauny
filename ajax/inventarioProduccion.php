...
require_once "../modelos/inventarioProduccion.php";

$inventarioProduccion=new inventarioProduccion(); 
...
switch ($_GET["op"]){
	...
	case 'generarPedido':  
		...
		$rspta=$inventarioProduccion->generarPedido($Deposito_idDeposito,$Categoria_idCategoria,$Proveedor_idProveedor,$Marca_idMarca,$Estado_idEstado);  
	...
	case 'listarAutorizacion':
		$rspta=$inventarioProduccion->listarAutorizacion();
 		...
 		$url='../reportes/exTicket.php?id=';
 			...
 			$data[]=array(
 				"0"=>'<button class="btn btn-primary" onclick="autorizarInventario('.$reg->idProduccion.')">Autorizar inventario <i class="fa fa-check"></i></button>',
 				"1"=>$reg->idProduccion,
 				"2"=>$reg->nombreDeposito,
 				"3"=>$reg->fechaTransaccion,
 				"4"=>$reg->usuario,
 				"5"=>$reg->cantidadTotal,   
 				"6"=>'<button class="btn btn-primary" onclick="mostrarDetalleAjuste('.$reg->idProduccion.')">Ver detalle de Produccion <i class="fa fa-pencil"></i></button>',
	...
	case 'autorizarInventario':
		...
		$rspta=$inventarioProduccion->autorizarInventario($idProduccion,$User);
 		$data= 1;
...
case 'mostrarDetalle':
		...
		$tipo=1;//$_REQUEST["tipo"];
		$contPrecio = 0;
		$rspta=$inventarioProduccion->mostrarDetalle($idProduccion);
 		//Vamos a declarar un array
 		$data= Array();
...
"8"=>'<input type="text" name="comentario[]"  style="width:100%;" onblur="actualizarComentario(this,\''.$reg->idcierrecostodetalle.'\')"  id="comentario[]" style="width:100%;" value="'.$reg->comentario.'" />',
	...
	case 'listar':
		...
		$rspta=$inventarioProduccion->listar($Deposito_idDeposito, $fi, $ff, $estado, $Sucursal_idSucursal, $Categoria_idCategoria);
 		//Vamos a declarar un array
 		$data= Array();
 		...
 		$url='../reportes/exTicket.php?id=';
 			$data[]=array(
 				...
 				"5"=>$reg->cantidadTotal,
 				"6"=>'<button class="btn btn-primary" onclick="mostrarDetalle('.$reg->idProduccion.')">Ver detalle <i class="fa fa-pencil"></i></button>',
	...
	case 'actualizarCantidadCaja':
		...
		$rspta=$inventarioProduccion->actualizarCantidadCaja($cantidad, $idArticulo,$Deposito_idDeposito);
 		//$data= 1;
 		echo json_encode($rspta);
