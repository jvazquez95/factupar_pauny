	...
	case 'listarAFacturar':
		...
		$rspta=$ordenVenta->listarAFacturar($habilitacion);
 		...
 		//$url='../reportes/exTicket.php?id=';
 			$data[]=array(
 				"0"=>
				'<button class="btn btn-warning" onclick="generarFactura(\''.$reg->idOrdenVenta.'\',\''.$reg->Deposito_idDeposito.'\', \''.$reg->TerminoPago_idTerminoPago.'\',\''.$reg->ovui.'\',\''.$reg->idPersona.'\',\''.$reg->contado.'\')"> Generar Factura</button>'.
				(((!$reg->ovi)?'':
 					'<button class="btn btn-warning" onclick="mostrar('.$reg->idOrdenVenta.')"><i class="fa fa-eye"></i></button>')
					)
					,
		...
		$rspta=$ordenVenta->listarFacturadosDirecta($habilitacion);
 		...
 		while ($reg=$rspta->fetchObject()){
 			if($reg->dd=='TICKET'){
				...
				// Botón Factura debe abrir el formato oficial
