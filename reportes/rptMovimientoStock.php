<?php
	...
	include 'plantillaMovimientoStock.php';
	...
	$pdf->Cell(20,6,'Codigo Barras',1,0,'C',1);
	$pdf->Cell(20,6,'Codigo Sap',1,0,'C',1);
	$pdf->Cell(20,6,'Codigo Interno',1,0,'C',1);
	...
	$pdf->SetFont('Arial','',6);
	$rpt = new MovimientoStock();
	//$rspta = $rpt->rpt_oc_detalle($_GET['empleado'],$_GET['fi'],$_GET['ff']);
	$rspta = $rpt->rpt_movimiento_stock($_GET['id']);
	$total = 0;
	$fila1 = 0;
	$saldoInicial = 0;
	while($row = $rspta->fetchObject())
	{
		...
		$pdf->Cell(20,6,utf8_decode($row->codigoBarra),1,0,'C');
		$pdf->Cell(20,6,$row->codigoAlternativo,1,0,'C');
		$pdf->Cell(20,6,$row->idArticulo.'',1,0,'C');			
		$pdf->Cell(60,6,$row->descripcion.'',1,0,'L');			
		$pdf->Cell(20,6,number_format($row->cantidad, 0, ',', '.').'.',1,0,'R');			
		$pdf->Cell(20,6,number_format($row->precio, 0, ',', '.').' Gs.',1,0,'R');			
		$pdf->Cell(20,6,number_format($row->total, 0, ',', '.').' Gs.',1,1,'R');	
	...
	$pdf->Cell(25,6,'Total General',1,0,'R',1);
