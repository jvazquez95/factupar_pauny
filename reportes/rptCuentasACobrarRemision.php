<?php
	...
	include 'plantillaCuentasACobrarRemision.php';
	...
	while($row = $rspta->fetchObject())  
	{
		...
		$totalRemisionPendiente = $totalRemisionPendiente +number_format($row->cantidadRemision, 0, ',', '.');
		$idPersonaAnterior=$row->razonSocial;    
	}
 
	$pdf->SetFont('Arial','B',8);

	$pdf->Cell(142,6,'Cantidad Total Pendiente de Remision:',1,0,'R',0); 
	$pdf->Cell(50,6,number_format($totalRemisionPendiente, 0, ',', '.').' ',1,0,'C');	
