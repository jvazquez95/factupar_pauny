<?php
	...
	$rspta = $rpt->rpt_recibo_detalle($_GET['idRecibo']);
	$total = 0;
	$fila1 = 0;
	$saldoInicial = 0;
	while($row = $rspta->fetchObject())
	{
		...
		$pdf->Cell(30,6,utf8_decode($row->serie. '-' .$row->nroFactura),1,0,'C');
		$pdf->Cell(27,6,$row->impCuota,1,0,'C');
	...
	$rpt = new Recibo();
	//$rspta = $rpt->rpt_oc_detalle($_GET['empleado'],$_GET['fi'],$_GET['ff']);
	$rspta = $rpt->rpt_recibo_detalle_pago($_GET['idRecibo']);
	$total = 0;
	$fila1 = 0;
	$usuarioInsercion = '';
	$fechaIns = '';
	...
	while($row = $rspta->fetchObject())
	{
		...
		$pdf->Cell(35,6,$row->emisor,1,0,'C');	
	...
	$pdf->Cell(27,6,'Recibimos de: ');	
	$pdf->Ln(15);
	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(27,6,'Firma:_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ Aclaracion:'.$_SESSION["razonSocial"].' C.I / RUC.:'.$_SESSION["nroDoc"]); 	
	$pdf->Ln(10);
