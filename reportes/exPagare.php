...
$empresa = 'FULL OFFICE - S.R.L.';
$propietario = 'Timbrado: ' .$reg0->timbrado;
$documento = "RUC: 80028600-6";
$direccion = $reg0->direccion;
$telefono = 'Tel.: ' .$reg0->telefono;
$email = 'Correo: ' .$reg0->correo;
error_reporting(0);
$date = date_create($reg->fechaVencimiento);
...
// $pdf->Cell(1,5,$propietario,0,2,"L");
// $pdf->Cell(1,5,$documento,0,2,"L");
// $pdf->Cell(1,5,$direccion,0,2,"L");
// $pdf->Cell(1,5,$email,0,2,"L");

// $pdf->Cell(50,20,'hola',1,0);
// $pdf->MultiCell(40,0, $email,1,"L");

//$pdf->Cell(1,5,"Vencimiento de timbrado " . date_format($date, 'd-m-Y'),0,2,"L");

$pdf->Cell(1);
$pdf->MultiCell(89,7,$empresa . "\n" .$propietario. "\n" . $documento. "\n" .$direccion. "\n" .$email,1,'L');
...
$pdf->SetY(10);
$pdf->SetX(100);
$pdf->MultiCell(100,7,$documento . "\n Total Gs: " .number_format($reg->total). utf8_decode("\nN° de Pagaré:").$reg->idVenta. "\n Factura Nro.:".$reg->serie."-".$reg->nroFactura."\n Fecha " .date_format($date1, 'd-m-Y')  ,1,'C'); 


// $pdf->Line(1,50, 100, 50);
$pdf->SetY(50);
$pdf->SetX(10);
//$pdf->Cell(100);
...
// $pdf->MultiCell(190,6,'Cantidad:' . $regd->cantidad . $regd->descripcionarticulo." ".$regd->impuesto."% \n P. unitario: ".$formattedNum = number_format($regd->precio)." Gs." ,1,'C');


$pdf->SetFont('Arial',"",8);

#$pdf->Cell(190,36, utf8_decode("El dia: ")  . date_format($date1, 'd-m-Y')."\n\n\n R.U.C: " .utf8_encode($reg->ruc)."adasjdjashdkjashkdjaskdhjakasjdasjkdjkasdjkasdjkashjkdhasjkdhasjkdhjaskhdjkashdjksahdjkashjkdasdhajshdjkashkjasdhjkadshkjadshjkasdhadjkshajkdshdaskjhdskjshakjhdsajkhdasjkhasdkjshdajkasdhkjdsajkhadskj\n\n\n\n", 1,0,'L');

#$pdf->SetY(56);
#$pdf->SetX(10);

$pdf->MultiCell(190,4, utf8_decode("\nPagaré a FULL OFFICE S.R.L o a su orden la suma de ") .numtoletras($reg->total).utf8_decode(" pagaderos en una entrega inicial de ").numtoletras($reg->entrega)." y ".$reg->cuotas." cuotas consecutivas de ".numtoletras((($reg->total-$reg->entrega)/$reg->cuotas))." a partir del ".date_format($date, 'd-m-Y').utf8_decode(". Por igual valor recibido en MERCADERIAS a mi entera satisfacción.").
utf8_decode("
La falta de pago a su vencimiento de cualquiera de las cuotas detalladas en el presente pagaré causará de pleno derecho el vencimiento anticipado de las cuotas de fechas no vencidas, quedando la acreedora facultada a exigir el pago inmediato del saldo total de la deuda, sin interpelación judicial o extrajudicial.
La mora producirá un interés de %___ de comisión y gastos. El simple vencimiento establecerá la mora, autorizando la inclusión de mi Nombre Personal o Razón Social que represento, a la base de datos de INFORMCONF conforme a lo establecido en la Ley 1682/01 como también para que se pueda proveer información a terceros interesados.
A todos los efectos legales y personales, acepto la jurisdicción de los tribunales de Caacupé y constituyo domicilio en Caacupé. Quedando entendido que cualquier notificación judicial o extrajudicial que en ello se pacifique será válida y se tendrá como efectuada personalmente.
\n
Deudor: ").utf8_decode($reg->nombreCliente)."                                                                                                                     Firma:__________________".
utf8_decode("\nDirección: ").$reg->direccion. utf8_decode("                                                                           \nC.I N°:").$reg->ruc. 
"\n                                                                                                                                                                                                                                             ". 
utf8_decode("\n                                                                                                                                                                 ")
...
$pdf->SetY(108.5);
$pdf->SetX(100);
$pdf->Cell(0,0, "CO-Deudor:" .$reg->codeudor,0,'L');
$pdf->SetY(115);
$pdf->SetX(100);
$pdf->Cell(0,0, "Firma:_________________________________",0,'L');
$pdf->SetY(122);
$pdf->SetX(100);
$pdf->Cell(0,0, utf8_decode("Dirección:_______________________________"),0,'L');
$pdf->SetY(128);
$pdf->SetX(100);
$pdf->Cell(0,0, utf8_decode("C.I N°:_________________________________"),0,'L');
...
#$pdf->MultiCell(60,4, 
#  "Cliente:" .utf8_decode($reg->nombreCliente) . "\nCajero: ".$reg->usuario. "\nCondicion: ".$condicionv. "\n RUC/C.I:".$reg->ruc. utf8_decode("\n Teléfono: ").$reg0->telefono,
#  1,'L');
