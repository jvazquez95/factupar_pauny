...
Class cotizacionDiaria
{
	...
	//Implementar un método para mostrar los datos de un registro a modificar
	public function selectCotizacionPorMoneda($valorMoneda)
	{
		$sql="SELECT a.cotizacionCompra as cotizacionCompra,b.idMoneda as moneda,b.descripcion as monedaN 
			FROM cotizaciondiaria a, moneda b
			where   a.Moneda_idMoneda=b.idMoneda and a.inactivo = 0 
			and a.Moneda_idMoneda = '$valorMoneda' order by a.idCotizacionDiaria desc limit 1";
	...
	/** Última cotización por cada moneda activa (para escritorio) */
	public function listarUltimasPorMoneda()
	{
		$sql = "SELECT m.idMoneda, m.descripcion, c.fecha, c.cotizacionVenta, c.cotizacionCompra
			FROM moneda m
			INNER JOIN cotizaciondiaria c ON c.Moneda_idMoneda = m.idMoneda AND c.inactivo = 0
			INNER JOIN (SELECT Moneda_idMoneda, MAX(idCotizacionDiaria) AS maxid FROM cotizaciondiaria WHERE inactivo = 0 GROUP BY Moneda_idMoneda) t ON t.Moneda_idMoneda = c.Moneda_idMoneda AND t.maxid = c.idCotizacionDiaria
			WHERE m.inactivo = 0
