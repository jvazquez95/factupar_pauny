...
Class ConciliacionCuentasCompra
{
	...
	public function insertar($Proveedor_idProveedor,$FECHARECIBO,$TOTAL, $Compra_idCompra,$montoAplicado,$nroCuota, $Compra_idCompraNC, $montoAplicadoNC)
	{
		...
		{
			$sql_detalle = "
					INSERT INTO `conciliacionCuentasCompra`
					(
					`Proveedor_idProveedor`,
					`Detalle_idDetalleCompraCuotas`,
					`cuota`,
					`saldo`,
					...
					(
						...
						'$Compra_idCompraNC[$num_elementos]',
						'$montoAplicadoNC[$num_elementos]',
						'$montoAplicadoNC[$num_elementos]',
						now(),
						'$usuario',
						0
	...
	{
		$sql="
		SELECT id, F_NOMBRE_PERSONA( Proveedor_idProveedor ) np, d.Compra_idCompra idCompra, d.monto ,d.saldo, n.idNotaCreditoCompra idNc, n.total, c.montoAplicado, nroCuota, c.inactivo as pci from conciliacionCuentasCompra c, detallecompracuotas d, notacreditocompra n 
		where 
		c.Detalle_idDetalleCompraCuotas = d.idDetalleCompraCuotas and 
		c.NotaCredito_idNotaCredito = n.idNotaCreditoCompra
		";
		return ejecutarConsulta($sql);		
	}
	...
	public function listarNotaCreditoCompra($idProveedor)
	{
		$sql="SELECT idNotaCreditoCompra, F_NOMBRE_PERSONA(Persona_idPersona) AS np, nroFactura ,total, fecha 
			  FROM notacreditocompra 
			  WHERE Compra_idCompra in ( 0, null ) and Persona_idPersona = '$idProveedor' and Persona_idPersona > 0";
		return ejecutarConsulta($sql);
	}
	...
	public function montoCuotaNC($id)
	{
		$sql="SELECT total from notacreditocompra where idNotaCreditoCompra = '$id' ";
		return ejecutarConsultaSimpleFila($sql);
	}
