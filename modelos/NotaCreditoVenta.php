...
Class NotaCreditoVenta
{
	...
	public function insertar($serie,$Persona_idPersona,$Habilitacion_idHabilitacion,$Deposito_idDeposito,$TerminoPago_idTerminoPago,$tipo_comprobante,$nroFactura,$fechaFactura,$timbrado,$vtoTimbrado,$totalImpuesto,$total,$totalNeto,$usuarioInsercion,$imagen,$venta_idVenta,$idarticulo,$cantidad,$precio,$impuesto,$importe_detalle,$tipopago,$nroReferencia, $tasaCambio, $tasaCambioBases, $Moneda_idMoneda)
	{

		$usuario = $_SESSION['login'];
			$sql = "INSERT INTO `notacreditoventa`
									( 
									`serie`, 
									`Persona_idPersona`, 
									`usuario`, 
		...
		{
			$totalv = $cantidad[$num_elementos] * $precio[$num_elementos];
			$netov = ( $totalv * $impuesto[$num_elementos] ) / 100;

			if ($num_elementos == $ultimo) {


			$sql_detalle = "INSERT INTO `notacreditoventadetalle`
							(
							`NotaCreditoVenta_idNotaCreditoVenta`,
							`Articulo_idArticulo`,
							`cantidad`,
							`precio`,
							/*`devuelve`,*/
...
*/
	...
	{
		$sql="UPDATE notacreditoventa SET inactivo=1 WHERE idNotaCreditoVenta='$idventa'";
		return ejecutarConsulta($sql);
	}
	...
	{
		$sql="SELECT a.idNotaCreditoVenta,a.nroDevolucion,a.nroFactura,c.razonSocial,a.tipoComprobante,a.fechaTransaccion,a.total as totalC,a.inactivo, a.Venta_idVenta
			from notacreditoventa a,venta b,persona c
			where a.Venta_idVenta=b.idVenta and a.Persona_idPersona=c.idPersona ;";
		return ejecutarConsulta($sql);	
	}
	...
	public function habilitacion()
	{
		...
		$sql="SELECT
				...
				where
				 ...
				 habilitacion.estado = 1 and deposito.Sucursal_idSucursal = sucursal.idSucursal and documentocajero.Usuario_idUsuario = habilitacion.Usuario_idUsuario and documentocajero.Documento_idTipoDocumento = 4";
