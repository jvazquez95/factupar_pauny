...
Class inventarioProduccion
{
	...
	public function autorizarInventario($idProduccion,$User)
	{
		$sql="UPDATE produccion SET estado=1, usuarioAprobacion='$User' WHERE idProduccion='$idProduccion' ";
		return ejecutarConsulta($sql);  
	}	
	...
	public function listarAutorizacion()
	{
		$sql="SELECT *, a.Deposito_idDeposito as cn , ifnull(b.descripcion,'TODOS') as nombreDeposito, a.inactivo as ci 
			  from produccion  a left outer join  deposito b on a.Deposito_idDeposito = b.idDeposito
	...
	public function generarPedidoArticulo($idinventarioProduccion, $Articulo_idArticulo, $Sucursal_idSucursal)
	{
		$usuario = $_SESSION['login'];

		$sql="CALL SP_InsertarArticuloinventarioProduccion('$idinventarioProduccion', '$Articulo_idArticulo', '$Sucursal_idSucursal')";
		return ejecutarConsulta($sql);
	}
	...
	public function generarOC($idinventarioProduccion)
	{
		$usuario = $_SESSION['login'];

		$sql="CALL SP_GenerarOC('$idinventarioProduccion')";
		return ejecutarConsulta($sql);
	}	
	...
	public function actualizarPedido($cantidad, $idinventarioProduccionDetalle)
	{
		$usuario = $_SESSION['login'];

		$sql="UPDATE ajustestockdetalle SET pedidos='$cantidad', usuarioModificacion='$usuario' WHERE idinventarioProduccionDetalle='$idinventarioProduccionDetalle'";
	...
	//Implementar un método para listar los registros
	public function listar($Deposito_idDeposito, $fi, $ff, $estado, $Sucursal_idSucursal, $Categoria_idCategoria)
	{

		/*$sql="CALL SP_ListarinventarioProduccion('$Deposito_idDeposito', '$fi', '$ff', '$estado', '$Sucursal_idSucursal', '$Categoria_idCategoria')";
		return ejecutarConsulta($sql);
			
			where  (a.Deposito_idDeposito= '$idDeposito' or '$idDeposito'=999)  and date(a.fechaTransaccion) between '$fi' and '$ff' 
	 			  and (a.Sucursal_idSucursal= '$Sucursal_idSucursal' or '$Sucursal_idSucursal'=999)
		*/ 
		$sql="SELECT a.idProduccion,b.descripcion as deposito,c.nombre as sucursal,d.nombre as categoria, a.fechaTransaccion,a.cantidadTotal,a.inactivo
					 from produccion a left outer join deposito b on a.Deposito_idDeposito=b.idDeposito 
					 left outer join sucursal c on b.Sucursal_idSucursal = c.idSucursal
					 left outer join categoria d on a.Categoria_idCategoria = d.idCategoria 
			   WHERE  (a.Deposito_idDeposito= '$Deposito_idDeposito' or '$Deposito_idDeposito'=999)   and (a.Categoria_idCategoria= '$Categoria_idCategoria' or '$Categoria_idCategoria'=999) and date(a.fechaTransaccion) between '$fi' and '$ff' and a.estado=0";	
	 	return ejecutarConsulta($sql);		
 	}
	...
	//Implementar un método para listar los registros
	public function mostrarDetalle($idinventarioProduccion)
	{

		 
			$sql="CALL SP_ListarinventarioProduccionDetalle('$idinventarioProduccion')";
