...
Class Articulo2
{
	...
	public function insertar($CuentaContable_idCuentaContable,$nombre,$descripcion,$codigo,$codigoBarra,$codigoAlternativo,$tipoArticulo,$GrupoArticulo_idGrupoArticulo,$Categoria_idCategoria,$Categoria_idCategoriaPadre,$TipoImpuesto_idTipoImpuesto,$Unidad_idUnidad,$Unidad_idUnidadCompra,$precioVenta,$usuarioInsercion,$imagen, $comisionp, $comisiongs ,$Persona_idPersona,$Marca_idMarca,$cantidadCaja,$pesoBruto,$pesoLiquido,$cantidadPiso,$cantidadPalet,$regimenTurismo,$balanza,$ventasKl,$costo, $idarticulo, $idarticulon , $cantidad, $comisionpp,$comisiongsp, $Sucursal_idSucursal, $GrupoPersona_idGrupoPersona, $precio,$margen,$Persona_idPersona_detalle, $prioridad, $codigoProveedor,$descontinuado,$precioCompra, $idArticulo_Proveedor, $codigoBarra_detalle, $descripcionCodigoBarra_detalle, $Unidad_idUnidad_detalle
		,$Articulo_idArticulo, $Canje, $cantidadMateriaPrima,$afectaCompra,$afectaVenta)
	{
			...
			while ($num_elementos < count($precio))
			{
				...
				$sql_detalle = "INSERT INTO `precio`
								...
								(
									...
									0, 
									'$usuarioInsercion',
									'$afectaCompra[$num_elementos]',
									'$afectaVenta[$num_elementos]'
								)";
...
if ( is_array( $idArticulo_Proveedor ) && $sw = true ) {
	...
	public function listarOriginal($x)
	{
		...
		$sql="SELECT *,articulo.nombre as na, articulo.descripcion as da, grupoarticulo.nombre as nga, articulo.inactivo as ea, marca.descripcion as md, F_NOMBRE_PERSONA(Persona_idPersona) as nprov, F_NOMBRE_UNIDAD(Unidad_idUnidadCompra) AS NUC, F_NOMBRE_UNIDAD(Unidad_idUnidad) as NUV, F_STOCK( articulo.idArticulo ) as stock, articulo.imagen as ai FROM articulo JOIN grupoarticulo ON grupoarticulo.idGrupoArticulo = articulo.GrupoArticulo_idGrupoArticulo JOIN marca ON articulo.Marca_idMarca = marca.idMArca JOIN (SELECT SUM(cantidad) as stock, Articulo_idArticulo FROM stock GROUP BY Articulo_idArticulo) AS s ON s.Articulo_idArticulo = articulo.idArticulo where (articulo.nombre like '$x' or articulo.descripcion like '$x' or articulo.codigoBarra like '$x' or articulo.codigo like '$x')  Order by articulo.inactivo asc, idArticulo DESC";
		return ejecutarConsulta($sql);	 	
	}
...
articulo.nombre as sn, detallearticulo.canje as canje, detallearticulo.cantidad as cantidad, detallearticulo.afectaCompra as compra,
detallearticulo.afectaVenta as venta
