...
Class HacerPedido
{
	...
	public function generarPedido($idProveedor, $meses,$Sucursal_idSucursal, $tipo)
	{
		$usuario = $_SESSION['login'];

		$sql="CALL SP_HacerPedido('$idProveedor', '$meses', '1', '$usuario', '$Sucursal_idSucursal', '$tipo')";
		return ejecutarConsulta($sql);
	}	


	public function generarPedidoArticulo($idHacerPedido, $Articulo_idArticulo, $Sucursal_idSucursal)
	{
		$usuario = $_SESSION['login'];

		$sql="CALL SP_InsertarArticuloHacerPedido('$idHacerPedido', '$Articulo_idArticulo', '$Sucursal_idSucursal')";
		return ejecutarConsulta($sql);
	}
	...
	public function generarOC($idHacerPedido)
	{
		$usuario = $_SESSION['login'];

		$sql="CALL SP_GenerarOC('$idHacerPedido')";
		return ejecutarConsulta($sql);
	}	


	public function actualizarCantidadCaja($cantidadCaja, $Articulo_idArticulo)
	{
		$usuario = $_SESSION['login'];

		$sql="UPDATE articulo SET cantidadCaja='$cantidadCaja', usuarioModificacion='$usuario' WHERE idArticulo='$Articulo_idArticulo'";
	...
	public function actualizarPedido($cantidad, $idHacerPedidoDetalle)
	{
		$usuario = $_SESSION['login'];

		$sql="UPDATE hacerpedidodetalle SET pedidos='$cantidad', usuarioModificacion='$usuario' WHERE idHacerPedidoDetalle='$idHacerPedidoDetalle'";
	...
	public function listar($idProveedor, $fi, $ff, $estado, $Sucursal_idSucursal)
	{
		$sql="SELECT *, persona.razonSocial as np, hacerpedido.inactivo as hpi from hacerpedido, persona where persona.idPersona = Persona_idPersona AND Persona_idPersona = '$idProveedor' and date(fecha) between '$fi' and '$ff'";
		return ejecutarConsulta($sql);		
	}
	...
	//Implementar un método para listar los registros
	public function mostrarDetalle($idHacerPedido, $tipo)
	{

		if ($tipo == 1) {
			$sql="CALL SP_ListarHacerPedidoDetalleSucursal('$idHacerPedido')";
		}else{
			$sql="CALL SP_ListarHacerPedidoDetalle('$idHacerPedido')";
		}
