...
function init(){
	...
	//Cargamos los items al select cliente
	$.post("../ajax/ventaRemision.php?op=selectBanco", function(r){
	...
	$.post("../ajax/ventaRemision.php?op=habilitacion", { tipoDocumento: 3 } ,function(data, status)
		{
	...
	//Habilitacion ticket
	$.post("../ajax/ventaRemision.php?op=ultimo", function(data, status)
		{
...
function cambiarComprobante(tipoDocumento){
	...
	//habilitacion factura
	$.post("../ajax/ventaRemision.php?op=habilitacion", { tipoDocumento: tipoDocumento } ,function(data, status)
		{
...
function mostrarExtracto(id){
	window.open("../reportes/rptCuentasACobrarRemision.php?fechai=2023-01-26&fechaf=2023-01-26&cliente="+id, "_blank");
}
...
function generarFactura(idOrdenVenta, Deposito_idDeposito, idTerminoPago, usuario, idPersona, contado){
	...
	$("#Deposito_idDeposito").selectpicker('refresh');
 	...
 	$.post("../ajax/ordenRemision.php?op=listarDetalleOrdenVenta&id="+idOrdenVenta,function(r){
	        $("#detalles").html(r);
...
function noCambiar(){
	...
	$.post("../ajax/terminoPago.php?op=selectTerminoPagoPersona", { idPersona: $("#Cliente_idCliente").val() } , function(r){
	            $("#TerminoPago_idTerminoPago").html(r);
	            $('#TerminoPago_idTerminoPago').selectpicker("refresh");

	            detallePago( $("#TerminoPago_idTerminoPago").val() );

	            	if ($("#TerminoPago_idTerminoPago").val() == 1) {
						$("#cuotas").val(1);
					}
	});	
...
function listar(habilitacion)
{
	tabla=$('#tbllistado').dataTable(
	{
		...
		"ajax":
				{
					url: '../ajax/ordenRemision.php?op=listarAFacturar',
...
function listarFacturados(habilitacion)
{
	tabla=$('#tbllistadoFacturados').dataTable(
	{
		...
		"ajax":
				{
					url: '../ajax/ordenRemision.php?op=listarFacturados',
					data:{habilitacion:habilitacion},
					type : "get",
					dataType : "json",						
					error: function(e){
...
function listarArticulos(Persona_idPersona0)
{
	...
	tabla=$('#tblarticulos').dataTable(
	{
		...
		"ajax":
				{
					url: '../ajax/ventaRemision.php?op=listarArticulosVenta',
...
function guardaryeditar(e)
{
	...
	$.ajax({
		url: "../ajax/ventaRemision.php?op=guardaryeditar",
	    type: "POST",
	    data: formData,
	    contentType: false,
	    processData: false,
	    ...
	    {                    
	        ...
	        mostrarform(false);
			window.open("../reportes/exFacturaFormRemision.php?id="+datos, "_blank");
			listar(l_habilitacion);
...
{
	$.post("../ajax/ventaRemision.php?op=mostrar",{idVenta : idVenta}, function(data, status)
	{
		data = JSON.parse(data);		
		mostrarform(true);
	...
	$("#tipo_comprobante").selectpicker('refresh');
 	...
 	$.post("../ajax/ventaRemision.php?op=listarDetalle&id="+idventa,function(r){
	        $("#detalles").html(r);
	});	
}
...
function anular(idVenta)
{
	bootbox.confirm("¿Está Seguro de anular la venta?", function(result){
		if(result)
        {
        	$.post("../ajax/ventaRemision.php?op=anular", {idVenta : idVenta}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});	
        }
...
function agregarDetalle(idarticulo,articulo,precioVenta,idImpuesto,impuesto, capital, interes, stock)
  {
  	var cantidad=1;
    var descuento=0;
	var precioVentan;
	...
	//alert( limiteAdd( precioVenta ));
	 ...
	 if (controlar_limite_credito == 1) {
		var terminoPago = $("#TerminoPago_idTerminoPago").val();
		if (terminoPago != 1) { 
			if (limiteAdd( precioVenta * cantidad ) > limite_credito) {
    ...
    if (idarticulo!="")
    {
    	...
    	'<td><input type="number" name="descuento[]" onblur="modificarSubototales()"  value="'+descuento+'" readonly></td>'+
  ...
  function modificarSubototales()
  {
    ...
    var cant_max = document.getElementsByName("cant_max[]");
    for (var i = 0; i <cant.length; i++) {
    	...
    	console.log(inpM.value);
    	if (parseInt(inpC.value) > parseInt(inpM.value)) {
    		alert('Remision supera el limite de venta');
			inpC.value = 0;
			inpC.focus(); 
    	}
    ...
    document.getElementsByName("cantmaxremision").innerHTML = maxremi;
  ...
  function evaluar(){
    // var detremi = document.getElementsByName("cantmaxremision").innerHTML;
    
    // console.log(detremi); 

    // if (detremi == 1 ){
    // 	alert('Ya superó el límite de cantidad de items para la remision asociada');
...
function agregarDetallePago()
{
	...
	var tipopagon= $("#FormaPago_idFormaPago option:selected").val();
  	...
  	var banco = $("#Banco_idBanco").val();
   	...
   	'<td><input type="text" name="banco[]" id="banco[]" value="'+banco+'"></td>'+
