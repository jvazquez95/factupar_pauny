...
var tabla2;
var l_habilitacion;
/** Multimoneda: 1 = Guaraníes (Gs.), 2 = Dólares (US$) */
var simboloMoneda = 'Gs. ';
var simboloMonedaCorto = 'Gs.';
var formatNumber = {
...
*/
	...
	//Cargamos los items al select moneda (Guaraníes / Dólares)
...
/** Actualiza símbolo de moneda según selector (1 = Gs., 2 = US$) y etiquetas en la vista */
function actualizarSimboloMoneda(){
	var id = $("#Moneda_idMoneda").val();
	if (id === '2' || id == 2) {
		simboloMoneda = 'US$ ';
		simboloMonedaCorto = 'US$';
	} else {
		simboloMoneda = 'Gs. ';
		simboloMonedaCorto = 'Gs.';
	}
	var h = document.getElementById('simboloMonedaHeader');
	var l = document.getElementById('simboloMonedaLabel');
	if (h) h.textContent = '(' + simboloMonedaCorto + ')';
	if (l) l.textContent = '(' + simboloMonedaCorto + ')';
}
...
function generarFactura(idOrdenVenta, Deposito_idDeposito, idTerminoPago, usuario, idPersona, contado){
	...
	//$.post("../ajax/persona.php?op=limiteCredito", { idPersona: g_idCliente } , function(data, status)
	//{
	//	data = JSON.parse(data);
...
function guardaryeditar(e)
{
	...
	$.ajax({
	    ...
	    {                    
	        ...
	        mostrarform(false);
			// Abrir el PDF de la factura en el formato oficial
...
function agregarDetalle(idarticulo,articulo,precioVenta,idImpuesto,impuesto, capital, interes, stock)
  ...
  function calcularTotales(){
	...
	impuestov = total - t1;
	$("#total2").html(simboloMoneda + separadorMiles(total));
    $("#total_venta").val(total);
	$("#total1").html(simboloMoneda + separadorMiles(t1));
    $("#total_ventan").val(t1);
