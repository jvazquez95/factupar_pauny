...
function agregarDetallePago()
{
	...
	if (monedan!=4){
		...
		$.post("../ajax/cotizacionDiaria.php?op=selectCotizacionPorMoneda", { monedan: y[i].value } , function(data, status)
		{ 
		...
		for(var i=0;i<y.length;i++) 
		{
			...
			if  ( y[i].value != 4 ) {
				...
				//var monedan= $("#Moneda_idMoneda2 option:selected").val();
			  	...
			  	var tasacambiodetalle2=0;

				data = JSON.parse(data);					
				var tasacambiodetalle2 = (data.cotizacionCompra);

			  	var tasacambiodetalle = tasacambiodetalle2;
			  	//var tasacambiodetalle = $("#tasaCambio2").val();
			  	var importe_detalle = $("#importe_detalle").val();
			  	var nroReferencia = $("#nroCheque").val();
