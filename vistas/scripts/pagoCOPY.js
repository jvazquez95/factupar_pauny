...
function init(){
	...
	$.post("../ajax/cheque.php?op=selectChequesPendientes", function(r){
...
function guardaryeditarCheque(e)
{
	...
	$.ajax({
	    ...
	    complete: function(datos){
			...
			$.post("../ajax/cheque.php?op=importe", {idCheque: datos.responseText} , function(data, status)
				{
...
function agregarDetalle(idarticulo,articulo,precioVenta,impuesto)
  {
    ...
    tipopago = $("#Moneda_idMoneda option:selected").val();
  	...
  	if (tipopago == 1) {
		...
		var lmonto = quitaSeparadorMilesPago($("#monto").val());		
  	}else{
	  	//var importe_detalle = $("#importe_detalle").val();
  ...
  function modificarSubototales()
  {
    ...
    for (var i = 0; i <neto.length; i++) {
    	var inpN=neto[i];
    	document.getElementsByName("monto")[i].innerHTML = quitaSeparadorMilesPago( inpN.value );
  ...
  function calcularTotales(x){
	...
	if (x == 0) {
	  	...
	  	for (var i = 0; i <sub.length; i++) {
			t1 = parseFloat(t1) + parseFloat(quitaSeparadorMilesPago(document.getElementsByName("monto")[i].value));
		}
