...
// 			$('#habilitacion').val(data.idhabilitacion);
// 			$('#habilitacion1').val(data.idhabilitacion);
// 			listar(data.idhabilitacion);
...
function reporte(id){
		window.open("../reportes/exReporteMovimiento.php?id="+ id);
}
...
function cerrar(idMovimiento)
{
	bootbox.confirm("¿Está Seguro de invalidar el movimiento?", function(result){
		if(result)
        {
        	$.post("../ajax/movimiento.php?op=desactivar", {idMovimiento : idMovimiento}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});	
        }
