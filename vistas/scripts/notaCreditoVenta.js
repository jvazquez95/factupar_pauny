...
function cargar(){
	...
	$.post("../ajax/venta.php?op=habilitacion", { tipoDocumento: 2 } , function(data, status)
		{
...
function cargarTasa(x){
	$.post("../ajax/cotizacionDiaria.php?op=ultimaCotizacion", { Moneda_idMoneda: x.value } , function(data, status)
	{
		...
		var tasa = data.cotizacionVenta != null ? Number(data.cotizacionVenta).toLocaleString() : '-';
...
function cargarCompra(x){

	//habilitacion factura
	$.post("../ajax/venta.php?op=datosVenta&id="+x.value, function(data, status)
		{

			data = JSON.parse(data);		


			g_idCliente = data.Cliente_idCliente;


		    $.ajax({
		    type: "POST",
		    url: "../ajax/persona.php?op=selectClienteLimit",
		    data:{keyword:data.Cliente_idCliente, tipoPersona: 1},
		    success: function(data){
		      $("select.selector_persona ").html(data);
		      $("select.selector_persona").selectpicker("refresh");
			...
			//$("#Persona_idPersona").val(data.Cliente_idCliente);
			//$("#Persona_idPersona").selectpicker("refresh");
 	...
 	$.post("../ajax/venta.php?op=listarDetalleVenta&id="+x.value,function(r){
	        $("#detalles").html(r);
	      //	detalle = 
...
function mostrarDetalleNC(idNotaCredito){
	$('#modal_detalle').modal('show');
	$("#detalle").val(idNotaCredito);
$(document).ready(function() {
    $('#tbllistado4').DataTable( {
		...
		"ajax":
				{
					url: '../ajax/consultas.php?op=rpt_ncv_detalle',
					data:{idNotaCredito:idNotaCredito},
					type : "get",
					dataType : "json",						
					error: function(e){
...
$.ajax({
    	type: "get",
    	url: '../ajax/venta.php?op=autorizarCompra',
    	data: {idCompra:idCompra},
		dataType:"json",
...
function listar()
{
	tabla=$('#tbllistado').dataTable(
	{
		"fnRowCallback": function( nRow, aaData, iDisplayIndex, iDisplayIndexFull ) {
		  if ( aaData[8] != '<span class="label bg-green">Aceptado</span>' )
		  {
		...
		"ajax":
				{
					url: '../ajax/notaCreditoVenta.php?op=listar',
					type : "get",
					dataType : "json",						
					error: function(e){
						console.log(e.responseText);	
...
function listarArticulos()
{
	tabla=$('#tblarticulos').dataTable(
	{
		...
		"ajax":
				{
					url: '../ajax/venta.php?op=listarArticulosCompra',
					type : "get",
					dataType : "json",						
					error: function(e){
						console.log(e.responseText);	
...
function guardaryeditar(e)
{
	...
	$.ajax({
		url: "../ajax/notaCreditoVenta.php?op=guardaryeditar",
	    type: "POST",
	    data: formData,
	    contentType: false,
	    processData: false,
...
{
	$.post("../ajax/venta.php?op=mostrar",{idcompra : idcompra}, function(data, status)
	{
		data = JSON.parse(data);		
		mostrarform(true);
...
function anular(idcompra)
{
	bootbox.confirm("¿Está Seguro de anular la venta?", function(result){
		if(result)
        {
        	$.post("../ajax/venta.php?op=anular", {idcompra : idcompra}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});	
        }
...
function agregarDetalle(idarticulo,articulo,costo,impuesto)
  {
  	var cantidad=1;
    var descuento=0;
    if (idarticulo!="")
    {
	    ...
	    var totalN = (costo - calculoImpuesto);    	
    	var subtotal=cantidad*costo;
    	var fila='<tr class="filas" id="fila'+cont+'">'+
    	'<td><button type="button" class="btn btn-danger" onclick="eliminarDetalle('+cont+')">X</button></td>'+
    	'<td><input type="hidden" name="idarticulo[]" value="'+idarticulo+'">'+articulo+'</td>'+
    	'<td><input type="text" onblur="modificarSubototales()" name="cantidad[]" id="cantidad[]" value="'+cantidad+'"></td>'+
    	'<td><input type="text onblur="modificarSubototales()"" name="precio[]" id="precio[]" value="'+costo+'"></td>'+
    	'<td><input type="text" onblur="modificarSubototales()" name="descuento[]" value="'+descuento+'"></td>'+
