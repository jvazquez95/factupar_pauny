...
function init(){
	...
	$.post("../ajax/produccion.php?op=selectDeposito", function(r){ 
...
function limpiar()
{
	...
	$("#costoTotal").val("");
	$("#cantidadTotal").val("");  
	$("#idProduccionDetalle").val(""); 
	$("#Produccion_idProduccion").val("");
...
function mostrar(idProduccion) 
{
	...
	$.post("../ajax/produccion.php?op=listarDetalle&idProduccion="+idProduccion,function(r){
	        $("#detalleProduccion").html(r);
	});	
}


function addDetalleProduccion()
  {
	...
	'<td><button type="button" class="btn btn-danger" onclick="eliminarDetalleProduccion ('+contT+', 0)">X</button></td>'+
	...
	$('#detalleProduccion').append(fila);
	//modificarSubototales();
  }


  function eliminarDetalleProduccion(indice,valor){

				if (valor > 0) {

				        	$.post("../ajax/produccion.php?op=desactivarDetalle", {idProduccionDetalle : valor}, function(e){
				        	});
				}	
...
swal({
	  ...
	  if (result.value) {
        	$.post("../ajax/produccion.php?op=desactivar", {idProduccion : idProduccion}, function(e){
			swal({
			  position: 'top-end',
			  type: 'success',
			  title: e,
...
swal({
	  ...
	  if (result.value) {
        	$.post("../ajax/produccion.php?op=activar", {idProduccion : idProduccion}, function(e){
			swal({
			  position: 'top-end',
			  type: 'success',
			  title: e,
