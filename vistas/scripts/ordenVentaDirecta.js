...
function buscar_cliente(Cliente_idCliente)
{
	...
	{ 
		...
		if (!data) {
			...
			$("#Cliente_idCliente").val( data.idPersona );
...
function guardaryeditar() //trabajo actual
{
...
{	
	...
	if (monedan!=4){
		...
		var y=document.getElementById("Moneda_idMoneda").options;
 
		for(var i=0;i<y.length;i++) 
		{

			if  ( y[i].innerHTML == monedat ) {

				//var monedan = y[i].value ;
				//var monedat = y[i].innerHTML;
				$("#valorMoneda").val(y[i].value);
				$("#valorMonedaN").val(y[i].innerHTML);
				
				var valorMoneda = $("#valorMoneda").val();

				$.post("../ajax/cotizacionDiaria.php?op=selectCotizacionPorMoneda", { valorMoneda: valorMoneda } , function(data, status)
				{ 
					data = JSON.parse(data);		
					$("#valorTasa").val(data.cotizacionCompra);
					$("#valorMoneda").val(data.moneda);	
					$("#valorMonedaN").val(data.monedaN);

					var tipopagon= $("#FormaPago_idFormaPago option:selected").val();
				  	var tipopagot = $("#FormaPago_idFormaPago option:selected").text();
					 
				  	var monedan = $("#valorMoneda").val();
				  	var monedat = $("#valorMonedaN").val();
				  	var tasacambiodetalle = $("#valorTasa").val();
				  	var importe_detalle = $("#importe_detalle").val();
				  	var nroReferencia = $("#nroCheque").val();

				  	var importeConMoneda = (importe_detalle/tasacambiodetalle);

				  	importeConMoneda = importeConMoneda.toFixed(2);
				   	...
				   	'<td><input type="number" name="importe_detalle[]" id="importe_detalle[]" value="'+importeConMoneda+'" readonly></td>'+
				   	'<td><input type="number" name="importe_detalle_edit[]" id="importe_detalle_edit[]" value="'+importeConMoneda+'" readonly ></td>'+
				   	'<td><input type="float" name="importe_detalle_pagado[]" id="importe_detalle_pagado[]"  value="'+0+'" onchange="modificarVueltoySaldo()"></td>'+
				   	'<td><input type="hidden" name="nroReferencia[]" id="nroReferencia[]" value="'+nroReferencia+'"></td>'+
				   	'<td><span name="subtotalDetalle" id="subtotalDetalle'+cont2+'">'+importeConMoneda+'</span></td>'+
				   	...
				   	$('#detalles12').append(fila2);

					var xy=document.getElementById("Moneda_idMoneda").options;
					...
					{				    	
				 		...
				 		if  ( xy[i].innerHTML == monedat ) {
						 	 
						 	var vueltoMoneda = xy[i].innerHTML;
						 	vueltoTotalD = 'Vuelto' + vueltoMoneda + ':';
					   		var fila2='<tr class="filas" id="fila2'+cont2+'">'+
					   		'<td></td>'+
					   		'<td></td>'+
					   		'<td></td>'+
					   		'<td></td>'+
					   		'<td></td>'+
						   	'<td><span name="vueltoTotal" id="vueltoTotal'+cont2+'">'+vueltoTotalD+'</span></td>'+
						   	'<td><input type="number" name="vueltoTotalM[]" id="vueltoTotalM[]" value="'+0+'"></td>'+
						   	'</tr>'; 
						   	cont2++;		
							$('#detalles12').append(fila2); 
						}		
...
function modificarVueltoySaldo()
{ 
  	var tipopago = document.getElementsByName("tipopago[]");
  	var importe_pagado = document.getElementsByName("importe_detalle_pagado[]");
    var importe_detalle = document.getElementsByName("importe_detalle[]");
    var ref = document.getElementsByName("nroReferencia[]");
    var sub = document.getElementsByName("subtotalDetalle");
    var saldo = document.getElementsByName("importe_detalle_edit[]");
    ...
    for (var i = 0; i <moneda.length; i++) {
    	...
    	var input2		=tipopago[i];
    	var inpPagado2	=importe_pagado[i];
    	var inpDetalle2	=importe_detalle[i];
    	var inpR2		=ref[i];
    	var inpS2 		=sub[i];
    	var inpSaldo2 	=saldo[i];
    	var inpTasa2 	=tasa[i];
    	sumaPagado=0;

    	for (var j = 0; j <importe_detalle.length; j++) {
	    	var input		=tipopago[j];
	    	var inpPagado	=importe_pagado[j];
	    	var inpDetalle	=importe_detalle[j];
	    	var inpR		=ref[j];
	    	var inpS 		=sub[j];
	    	var inpSaldo 	=saldo[j];
	    	var inpTasa 	=tasa[j];


			//sumaPagado  = parseInt(sumaPagado) + ( parseInt(inpPagado.value) * parseInt(inpTasa.value) );
	 		sumaPagado  = sumaPagado + ( parseFloat(inpPagado.value).toFixed(2) * parseFloat(inpTasa.value).toFixed(2) );
	 		//sumaPagado = parseInt(sumaPagado);
	 		//sumaPagado = parseInt(sumaPagado).toFixed(2);
	    } 
  		 
	    console.log(sumaPagado);
    	inpSaldo2.value = (parseFloat(inpDetalle2.value).toFixed(2)) - ( (parseFloat(sumaPagado).toFixed(2)	 / parseFloat(inpTasa2.value).toFixed(2)) ); 
  
  		inpSaldo2.value = parseFloat(inpSaldo2.value).toFixed(2);   


  		if (inpSaldo2.value < 0){
  			inpSaldo2.value = 0;
  		}


    	document.getElementsByName("importe_detalle_edit[]")[i].value = inpSaldo2.value;

    }

   calcularTotalesDetalle12(marcaEntra);
}

function modificarSubototalesDetalle12()
{
    ...
    for (var i = 0; i <tp.length; i++) {
    	...
    	//inpS.value = parseFloat(inpI.value * inpM.value) ;
    	inpS.value = parseFloat(inpI.value * inpM.value).toFixed(2);
 ...
 function calcularTotalesDetalle12(marcaEntra){

  	var imp = document.getElementsByName("subtotalDetalle"); 
  	var impPagado = document.getElementsByName("importe_detalle_pagado[]");
  	var tasa = document.getElementsByName("tasa[]");
  	var total = 0.0; 
	var totalPagado = 0.0;     
	var totalVuelto = 0.0;
  	...
  	for (var i = 0; i <imp.length; i++) {
		total = parseFloat(document.getElementsByName("subtotalDetalle")[0].value); 
		totalPagado += (parseFloat(document.getElementsByName("importe_detalle_pagado[]")[i].value) * parseFloat(document.getElementsByName("tasa[]")[i].value));
	}
	...
	if (totalPagado  >= total){
		...
		totalVuelto = parseInt(totalPagado  - total);
	...
	if (marcaEntra == 1) {
		for (var i = 0; i <imp.length; i++) {
			document.getElementsByName("vueltoTotalM[]")[i].value = parseFloat( parseInt(totalVuelto) / parseInt(tasa[i].value)).toFixed(2);	
		}
 	}
		$("#total_detalle12").html("Gs. " + total);
		$("#totalVuelto_detalle12").html("Gs. " + totalVuelto);	
    	$("#t_detalle12").val(total); 
  		$("#tv_detalle12").val(totalVuelto);
		
	/*	$("#vueltoTotalM")[3].html("Gs. " + totalVuelto + tasa[0].value);
  	 	$("#vueltoTotalM")[3].val(totalVuelto+ tasa[0]);

		$("#vueltoTotalM")[4].html("Gs. " + totalVuelto + tasa[1].value);
  	 	$("#vueltoTotalM")[4].val(totalVuelto+ tasa[1]);

		$("#vueltoTotalM")[5].html("Gs. " + totalVuelto + tasa[2].value);
  	 	$("#vueltoTotalM")[5].val(totalVuelto+ tasa[2]);*/

  		//$("#vueltoTotalM").html("Gs. " + totalVuelto);
  		//$("#vueltoTotalM").val(totalVuelto);
