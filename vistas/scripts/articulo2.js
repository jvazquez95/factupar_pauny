...
function validarCodigo(x){
	var codigo = x.value;
	//Habilitacion ticket
	$.post("../ajax/articulo2.php?op=validarCodigo", {codigo:codigo} ,function(data, status)
		{
...
function validarCodigoBarra(x){
	...
	$.post("../ajax/articulo2.php?op=validarCodigoBarra", {codigo:codigo}, function(data, status)
		{
...
$("#paquetedetalle").hide();
	...
	//Cargamos los items al select grupo
	$.post("../ajax/articulo2.php?op=selectGrupo", function(r){
	...
	$.post("../ajax/articulo2.php?op=selectCategoria", function(r){
	...
	$.post("../ajax/articulo2.php?op=selectImpuesto", function(r){
	...
	$.post("../ajax/articulo2.php?op=selectUnidad", function(r){
 	...
 	//Cargamos los items al select unidad
	$.post("../ajax/articulo2.php?op=selectArticulos", function(r){
...
{  
	...
	//Cargamos los items al select categoria
		...
		$.post("../ajax/articulo2.php?op=selectCategoriaDetalle&Categoria_idCategoria="+Categoria_idCategoria, function(r){
		    $("#Categoria_idCategoriaD").html(r);
		    $('#Categoria_idCategoriaD').selectpicker('refresh'); 
		});
	});
...
$(document).ready(function() {
    $('#tbllistado4').DataTable( {
		...
		"ajax":
				{
					url: '../ajax/articulo2.php?op=detalle',
...
//RECORRIDO POR GRUPO DE SUCURSAL - COMENTAR PARA NO VALIDAR
  ...
  function eliminarDetalleCodigoBarra(indice, valor){
        	...
        	$.post("../ajax/articulo2.php?op=desactivarArticuloCodigo", {idArticulo_Codigo : valor}, function(e){
...
function addDetalleMateriaPrima()
  {
	...
	var phpCompra = document.getElementsByName("compra_compraid[]");
	var phpVenta = document.getElementsByName("venta_ventaid[]");
	//RECORRIDO POR GRUPO DE PERSONA - COMENTAR PARA NO VALIDAR
   ...
   /* for (var index = 0; index < document.getElementsByName("Canje[]").length; index++) {
	...
	var Articulo_idArticulo = $("#Articulo_idArticulo_l option:selected").val();
  	...
  	var compras = $("#compra option:selected").val();
  	var compra_text = $("#compra option:selected").text();

  	var ventas = $("#venta option:selected").val();
  	var venta_text = $("#venta option:selected").text();
	...
	'<td><input type="hidden" name="compra_compraid[]" value="'+compras+'">'+compra_text+'</td>'+
	'<td><input type="hidden" name="venta_ventaid[]" value="'+ventas+'">'+venta_text+'</td>'+
	'</tr>'; 
	contMateriaPrima++; 
  ...
  function eliminarDetalleMateriaPrima(indice, valor){
        	...
        	$.post("../ajax/articulo2.php?op=desactivarMateriaPrima", {idArticulo_Detalle : valor}, function(e){
  ...
  function eliminarDetallePrecio(indice, valor){
        	...
        	$.post("../ajax/articulo2.php?op=desactivarPrecio", {idPrecio : valor}, function(e){
...
//RECORRIDO POR GRUPO DE SUCURSAL - COMENTAR PARA NO VALIDAR
  ...
  function eliminarDetalleProveedor(indice, valor){
        	...
        	$.post("../ajax/articulo2.php?op=desactivarArticuloProveedor", {idArticulo_Proveedor : valor}, function(e){
...
function listarOriginal()
{
	...
	tabla=$('#tbllistado').dataTable(
	{
		...
		"ajax":
				{
					url: '../ajax/articulo2.php?op=listarOriginal',
					type : "get",
					data: { x : x },
					dataType : "json",						
					error: function(e){
...
function listar()
{
	    ...
	    buttons: [		          
              ...
              "serverSide": true,
              "responsive": true,
              "sAjaxSource": "../ajax/articulo2.php?op=listar",
              "columnDefs":[{
                  "data":null
              }]   
           }); 
...
function guardaryeditar(e)
{
	...
	if (contPrecio > 0  || $("#idArticulo").val() > 1 ) {
		...
		if (contCodigoBarra > 0 || $("#idArticulo").val() > 1 ) {
			...
			$.ajax({
				url: "../ajax/articulo2.php?op=guardaryeditar",
			    type: "POST",
			    data: formData,
			    contentType: false,
			    processData: false,
...
function mostrar(idArticulo)
{
	...
	$.post("../ajax/articulo2.php?op=mostrar",{idArticulo : idArticulo}, function(data, status)
	{
		data = JSON.parse(data);		
		mostrarform(true);
	...
	$("#idArticulo").val(data.idArticulo);
 	...
 	$.post("../ajax/articulo2.php?op=listarDetallePrecio&idArticulo="+idArticulo,function(r){
	        $("#detalle").html(r);
	});	

 	$.post("../ajax/articulo2.php?op=listarDetalleMateriaPrima&idArticulo="+idArticulo,function(r){
	        $("#detalleMateriaPrima").html(r); 
	});		

 	$.post("../ajax/articulo2.php?op=listarDetalleArticuloProveedor&idArticulo="+idArticulo,function(r){
	        $("#detalleProveedor").html(r);
	});	


 	$.post("../ajax/articulo2.php?op=listarDetalleCodigoBarras&idArticulo="+idArticulo,function(r){
...
function desactivar(idArticulo)
{
	bootbox.confirm("¿Está Seguro de desactivar el artículo?", function(result){
		if(result)
        {
        	$.post("../ajax/articulo2.php?op=desactivar", {idArticulo : idArticulo}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});	
        }
...
function activar(idArticulo)
{
	bootbox.confirm("¿Está Seguro de activar el Artículo?", function(result){
		if(result)
        {
        	$.post("../ajax/articulo2.php?op=activar", {idArticulo : idArticulo}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});	
        }
...
function actualizarGs(x, id, servicio){
	...
	$.ajax({
    type: "POST",
    url: '../ajax/articulo2.php?op=ajuste_actualizar_gs',
...
function actualizarP(x, id, servicio){
	...
	$.ajax({
    type: "POST",
    url: '../ajax/articulo2.php?op=ajuste_actualizar_p',
...
function actualizarC(x, id, servicio){
	...
	$.ajax({
    type: "POST",
    url: '../ajax/articulo2.php?op=ajuste_actualizar_c',
