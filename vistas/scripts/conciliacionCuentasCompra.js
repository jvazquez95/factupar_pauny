...
function cargarFC(){
	...
	$.post("../ajax/conciliacionCuentasCompra.php?op=selectNotaCreditoCompra",{lProveedor:lProveedor}, function(r){
	            $("#NotaCredito_idNotaCredito").html(r);
	            $('#NotaCredito_idNotaCredito').selectpicker('refresh');
	});	
...
function cargarNCV(){
	...
	var lidCompra = $('#Compra_idCompra').val();
	//Cargamos los items al select cliente
	$.post("../ajax/conciliacionCuentasCompra.php?op=selectCuotasCompra",{lidCompra:lidCompra}, function(r){
	            $("#nroCuota").html(r);
	            $('#nroCuota').selectpicker('refresh');
...
function montoCuotaNC(){
	var idNotaCreditoCompra = $('#NotaCredito_idNotaCredito').val();


	$.post("../ajax/conciliacionCuentasCompra.php?op=montoCuotaNC",{idNotaCreditoCompra:idNotaCreditoCompra}, function(data, status)
		{

			data = JSON.parse(data);
			$('#montoAplicar').val(data.total);
...
function listar()
{
	...
	{		
		"fnRowCallback": function( nRow, aaData, iDisplayIndex, iDisplayIndexFull ) {
		  if ( aaData[9] != '<span class="label bg-green">Activado</span>' )
		  {
		...
		"ajax":
				{
					url: '../ajax/conciliacionCuentasCompra.php?op=listar',
					type : "get",
					dataType : "json",						
					error: function(e){
						console.log(e.responseText);	
...
function guardaryeditar(e)
{
	...
	$.ajax({
		url: "../ajax/conciliacionCuentasCompra.php?op=guardaryeditar",
	    type: "POST",
	    data: formData,
	    contentType: false,
	    processData: false,
...
function agregarDetalle(idarticulo,articulo,precioVenta,impuesto)
  {
	...
	var lNotaCredito_idNotaCredito = $("#NotaCredito_idNotaCredito option:selected").val();
  	var lfactura = $("#Compra_idCompra option:selected").text();
  	var lNotaCredito_idNotaCredito_text = $("#NotaCredito_idNotaCredito option:selected").text();
	...
	var lmonto = Math.abs(parseFloat($("#monto").val()));
	var lmontoNC = Math.abs(parseFloat($("#montoAplicar").val()));
	...
	if (lmonto >= lmontoNC) {
		...
		swal({
		  position: 'top-end',
		  type: 'danger',
		  title: "Saldo de Nota de Credito no puede ser mayor al saldo de la Factura! Verificar",
		  showConfirmButton: false,
		  timer: 1500
	    })
	}
	...
	//var lempleado_nombre = $("#Empleado_idEmpleado option:selected").text();
    	...
    	'<td><input type="text" name="montoAplicado[]" id="montoAplicado'+cont+'" value="'+lmonto+'"></td>'+
    	'<td><input type="hidden" name="Compra_idCompraNC[]" value="'+lNotaCredito_idNotaCredito+'">'+lNotaCredito_idNotaCredito_text+'</td>'+
    	'<td><input type="number" name="montoAplicadoNC[]" id="montoAplicadoNC'+cont+'" readonly value="'+lmontoNC+'"></td>'+
