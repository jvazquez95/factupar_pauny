...
function init(){
	...
	//Cargamos los items al select compras
	...
	$.post("../ajax/compra.php?op=habilitacion", function(data, status)
		{
			...
			$('#Deposito_idDeposito').val(3);
			serie = data.serie;
			fra = data.a;

			//$("#Deposito_idDeposito").val(data.dp);
			//$("#Deposito_idDeposito1").val(data.deposito);
			$("#Deposito_idDeposito1").val(3);
...
function cargarCompra(x){
	...
	//habilitacion factura
	$.post("../ajax/compra.php?op=datosCompra&id="+x.value, function(data, status)
		{
 	...
 	$.post("../ajax/compra.php?op=listarDetalleCompra&id="+x.value,function(r){
	        $("#detalles").html(r);
...
$(document).ready(function() {
    $('#tbllistado4').DataTable( {
		...
		"ajax":
				{
					url: '../ajax/consultas.php?op=rpt_nc_detalle',
...
function agregarDetalle(idarticulo,articulo,costo,impuesto)
  ...
  function calcularTotales(){
	...
	impuestov = total - t1;
	$("#total2").html("Gs. " + total.toFixed(2));
    $("#total_compra").val(total.toFixed(2));
	$("#total1").html("Gs. " + t1.toFixed(2));
