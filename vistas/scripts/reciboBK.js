...
function agregarDetalle(idarticulo,articulo,precioVenta,impuesto)
  {
	...
	//var lempleado_nombre = $("#Empleado_idEmpleado option:selected").text();
    	...
    	'<td><input type="hidden" name="montoAplicado[]" id="montoAplicado[]" value="'+lmonto+'">'+lmonto+'</td>'+
  ...
  function modificarSubototales()
  {
    ...
    for (var i = 0; i <neto.length; i++) {
    	var inpN=neto[i];
		  	...
		  	if (tipopago == 1) {
			  	//var importe_detalle=$("#importe_detalle").val();
				//t1 = parseFloat(t1) + quitaSeparadorMiles(parseFloat(document.getElementsByName("monto")[i].value));
...
function agregarDetallePago()
{
    ...
    tipopago= $("#Moneda_idMoneda option:selected").val();
  	...
  	var nroReferencia = $("#nroCheque").val();
   	...
   	'<td><input type="hidden" name="tipopago[]" value="'+l_tipopago+'">'+tipopagot+'</td>'+
   	'<td><input type="text" name="importe_detalle[]" id="importe_detalle[]" value="'+importe_detalle+'"></td>'+
