...
function init(){
	...
	$.post("../ajax/cheque.php?op=selectChequesPendientesRecibidos", function(r){
	...
	$.post("../ajax/venta.php?op=habilitacion",{ tipoDocumento: 1 } , function(data, status)
		{
        	...
        	$("#serie1").val(serie); 
        	$("#nroFactura").val(fra); 
			$("#FECHARECIBO1").val(data.fecha);
			$("#FECHARECIBO").val(data.fecha);
			$("#Deposito_idDeposito").val(data.dp);
...
function guardaryeditarCheque(e)
{
	...
	$.ajax({
		url: "../ajax/cheque.php?op=guardarChequeRecibido",
	    type: "POST",
	    data: formData,
	    contentType: false,
	    processData: false,
	    ...
	    complete: function(datos){
			$('#cheque').modal('hide');
			$("#nroCheque").val(datos.responseText);
			$.post("../ajax/cheque.php?op=importeRecibidos", {idCheque: datos.responseText} , function(data, status)
				{
...
$("#nroCheque").val(nro);
			$.post("../ajax/cheque.php?op=importeRecibidos", {idCheque: nro} , function(data, status)
				{
					data = JSON.parse(data);	
					$("#importe_detalle").val(data.monto);
			 });	
...
$(document).ready(function() {
    $('#tbllistado5').DataTable( {
		...
		"ajax":
				{
					url: '../ajax/consultas.php?op=rpt_cobros_detalle_recibo',
...
function cargarTasa(x){
	$.post("../ajax/cotizacionDiaria.php?op=ultimaCotizacion", { Moneda_idMoneda: x.value } , function(data, status)
	{
		data = JSON.parse(data);
		$("#tasaCambio").val(data.cotizacionCompra);
		var monedaText = $(x).find("option:selected").text();
		var tasa = data.cotizacionCompra != null ? Number(data.cotizacionCompra).toLocaleString() : '-';
		$("#labelCambioMoneda").text("1 " + monedaText + " = " + tasa + " Gs.");
	});
	$.post("../ajax/cotizacionDiaria.php?op=ultimaCotizacion", { Moneda_idMoneda: 2 } , function(data, status)
	{
		data = JSON.parse(data);
...
function cargarCV(){
	...
	var lidVenta = $('#Venta_idVenta').val();
	//Cargamos los items al select cliente
	$.post("../ajax/recibo.php?op=selectCuotasVenta",{lidVenta:lidVenta}, function(r){
	            $("#nroCuota").html(r);
...
function montoCuota(){
	...
	$.post("../ajax/recibo.php?op=montoCuota",{lidVenta:lidVenta,lcuota:lcuota}, function(data, status)
		{
...
function agregarDetalle(idarticulo,articulo,precioVenta,impuesto)
  {
	...
	var lVenta_idVenta = $("#Venta_idVenta option:selected").val();
  	var lfactura = $("#Venta_idVenta option:selected").text();

	var lnroCuota = $("#nroCuota option:selected").val();
  	//var lservicio_nombre = $("#Articulo_idArticulo_Servicio option:selected").text();

	if ( $("#Moneda_idMoneda option:selected").val() == 1 ) {
		var lmonto = quitaSeparadorMiles($("#monto").val());
	}else{
	...
	//var lempleado_nombre = $("#Empleado_idEmpleado option:selected").text();
    	...
    	'<td><input type="hidden" name="Venta_idVenta[]" value="'+lVenta_idVenta+'">'+lfactura+'</td>'+
    	'<td><input type="hidden" name="nroCuota[]" value="'+lnroCuota+'">'+lnroCuota+'</td>'+
    	'<td><input type="hidden" name="montoAplicado[]" id="montoAplicado'+cont+'" value="'+lmonto+'">'+lmonto+'</td>'+
  ...
  function modificarSubototales()
  {
    ...
    for (var i = 0; i < neto.length; i++) {
		...
		if ( $("#Moneda_idMoneda option:selected").val() == 1 ) {
    		document.getElementsByName("monto")[i].innerHTML = quitaSeparadorMiles(inpN.value);
		}else{
  ...
  function calcularTotales(x){
	...
	if (x == 0) {
	  	...
	  	for (var i = 0; i <sub.length; i++) {
			...
			if ( $("#Moneda_idMoneda option:selected").val() == 1 ) {
				t1 = parseFloat(t1) + parseFloat(quitaSeparadorMiles(document.getElementsByName("monto")[i].value));
			}else{
				t1 = parseFloat(t1) + parseFloat((document.getElementsByName("monto")[i].value));
			}
		}
		...
		t1 = parseFloat(t1) - parseFloat(restarEsto);
...
function agregarDetallePago()
{
	...
	var tipopago= $("#TerminoPago_idTerminoPagoDetalle option:selected").val();
  	...
  	if (tipopago == 4) {
	  	...
	  	var importe_detalle = quitaSeparadorMiles($("#importe_detalle").val());
  	}
