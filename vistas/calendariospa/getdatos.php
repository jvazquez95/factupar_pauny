...
if(isset($_POST['action']) or isset($_GET['view']) or isset($_GET['empleados']) or isset($_GET['action']))
{
    if(isset($_GET['view'])){
        ...
        $result = mysqli_query($connection,"    SELECT ordenconsumisiondetalle.idOrdenConsumisionDetalle as id,  F_NOMBRE_CLIENTE(ordenconsumision.Cliente_idCliente) as title, F_NOMBRE_ARTICULO(ordenconsumisiondetalle.Articulo_idArticulo) as body, ordenconsumisiondetalle.Articulo_idArticulo as idarticulo, F_NOMBRE_SERVICIO( clientedetalle.Articulo_idArticulo_Servicio ) as servicio, (select nombre from paquetedetalle where idpaqueteDetalle = ordenconsumisiondetalle.Articulo_idArticulo_Servicio ) as paquete, 'event-info' as class, ordenconsumisiondetalle.Empleado_IdEmpleado as empleadoactual,  fecha_inicial as start, fecha_final as end from ordenconsumision, ordenconsumisiondetalle, clientedetalle where ordenconsumision.idOrdenConsumision = ordenconsumisiondetalle.OrdenConsumision_idOrdenConsumision AND ordenconsumisiondetalle.Empleado_IdEmpleado = ".$getempleado." and ordenconsumision.inactivo = 0 and ordenconsumisiondetalle.inactivo = 0 ORDER BY id DESC LIMIT ".$getidurl.";    ");
    ...
    if (!empty($servicio)) {
        ...
        mysqli_query($connection, "INSERT INTO ordenconsumision (`idOrdenConsumision`, `Empleado_idEmpleado`, `fechaConsumision`, `Cliente_idCliente`, `inactivo`, `terminado`, `usuarioIns`) VALUES (NULL, '$idempleado', '', '$cliente_id', '0', '0', 'MARINA');");
        $addid = mysqli_insert_id($connection);
        mysqli_query($connection, "INSERT INTO `ordenconsumisiondetalle` VALUES (NULL, '$addid', '$articulo_id', '$servicio', '$idempleado', '$cantidad','02', '0', NULL, '".mysqli_real_escape_string($connection,date('Y-m-d H:i:s',strtotime($_POST["start"])))."',  '".mysqli_real_escape_string($connection,date('Y-m-d H:i:s',strtotime($_POST["end"])))."', '$sala');");
        $addid = mysqli_insert_id($connection);
        header('Content-Type: application/json');
    ...
    $idcliente =  $_POST['idcliente'];
     mysqli_query($connection,"UPDATE `ordenconsumisiondetalle` SET `Empleado_IdEmpleado` = '".$idempleado."' WHERE `idOrdenConsumisionDetalle` = " .$_POST['id']. ";");
    header('Content-Type: application/json');
